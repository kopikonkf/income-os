...
| `die-lnx-executive-001` | `company_portfolio` | Yes | Linux dedicated Chrome profile; dynamic loopback CDP | dynamic loopback CDP | `127.0.0.1:8891` staging (18 tools) | same shared Executive role contract inside DIE-LINUX | Windows account/session, shell, Git, state writes |
| `die-lnx-division-001` | `single_division:DIVISION-01` | Yes | Linux dedicated Chrome profile; dynamic loopback CDP | dynamic loopback CDP | `127.0.0.1:8892` staging (6 tools) | same shared Division01 role contract inside DIE-LINUX | DIE-WINDOWS Executive, other divisions, portfolio capital |
...
- Decision MCPs (`:8791`, `:8792`) are DIE-WINDOWS principal-pinned; Linux staging MCPs (`:8891`, `:8892`) are separately pinned to `die-lnx-executive-001` / `die-lnx-division-001`; `:8787` is Architect DEV and forbidden to all runtime identities.
