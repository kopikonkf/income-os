...
const pages=[0,1,2,3,4].map(i=>({{i,closed:false,isClosed(){{return this.closed}},async close(){{this.closed=true}}}}));
...
console.log(JSON.stringify({{r,open:context.pages().map(p=>p.i),preservedClosed:pages[0].closed}}));'''
    ...
    assert v['r']=={'before':5,'after':2,'closed':3}
    assert v['preservedClosed'] is False
    assert len(v['open'])==2 and 0 in v['open']

def test_persistent_principals_enforce_two_tab_budget_and_rotation_reclaims_tabs():
