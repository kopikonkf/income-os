...
def test_wrk001_bounded_model_receipt_records_exact_identity_without_fallback(tmp_path: Path) -> None:
    ...
    fake.write_text("#!/usr/bin/env python3\nimport json\nprint(json.dumps({'type':'text','part':{'text':'WRK001_BOUNDED_OK'}}))\nprint(json.dumps({'type':'step_finish','part':{'reason':'stop','cost':0,'tokens':{}}}))\n", encoding="utf-8")
...
@pytest.mark.skipif(os.name == "nt", reason="lingering-runtime subprocess proof is Linux-only")
def test_wrk001_accepts_durable_completion_and_tears_down_lingering_runtime(tmp_path: Path) -> None:
    workspace_root = tmp_path / "workspaces"
    workspace = workspace_root / "job"
    workspace.mkdir(parents=True)
    worker_home = tmp_path / "worker-home"
    ...
    fake.write_text(
        "#!/usr/bin/env python3\n"
        "import json,time\n"
        "print(json.dumps({'type':'text','part':{'text':'WRK001_BOUNDED_OK'}}), flush=True)\n"
        "print(json.dumps({'type':'step_finish','part':{'reason':'stop','cost':0,'tokens':{}}}), flush=True)\n"
        "time.sleep(20)\n",
        encoding="utf-8",
    )
    fake.chmod(fake.stat().st_mode | stat.S_IXUSR)
    ...
    policy["binary"] = str(fake)
    receipt = MW.execute(
        job=job(workspace), policy=policy, workspace_root=workspace_root,
        expected_marker="WRK001_BOUNDED_OK", timeout_sec=5,
    )
    assert receipt["status"] == "PASS"
    assert receipt["completion_event_observed"] is True
    assert receipt["completion_reason"] == "stop"
    assert receipt["observed_cost"] == 0
    assert receipt["duration_ms"] < 5000
    assert receipt["process_cleanup"] in {"TERMINATED_AFTER_COMPLETION", "KILLED_AFTER_COMPLETION"}
