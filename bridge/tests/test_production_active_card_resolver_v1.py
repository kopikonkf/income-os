...
def test_blueprint_ready_remains_autonomously_actionable(tmp_path:Path):
    progress(tmp_path,'READY','Seed: SEED-000030 (ready)','State: BLUEPRINT_READY','Started: 2026-09-02T00:00:00Z')
    out=mod.resolve_active_card(tmp_path)
    assert out['status']=='CONTINUE_ACTIVE_CARD'
    assert out['active_card']['execution_ready'] is True
    assert out['active_card']['next_action_type']=='PROD-DISPATCH-WORKER'
