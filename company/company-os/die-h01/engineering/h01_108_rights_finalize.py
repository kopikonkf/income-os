...
def main():
 ...
 rec=json.loads((w/'asset-receipt.json').read_text());rec['rights']=rights['result'];rec['rights_signal_gate']=rights['result'];rec['visual_rights_self_test']=det['runtime']['self_test_result'];rec['postproduction_classification']='PASS' if rights['result']=='PASS' else ('REVIEW_REQUIRED' if rights['result']=='REVIEW_REQUIRED' else 'BLOCKED_RIGHTS');rec['status']='ACCEPTED_SEMANTIC_MASTER' if rights['result'] in {'PASS','REVIEW_REQUIRED'} else 'BLOCKED_RIGHTS';rec['generation_status']='GENERATION_COMPLETE';rec['generation_validity_effect']='NONE';rec['submission_eligible']=False;dump(w/'asset-receipt.json',rec)
