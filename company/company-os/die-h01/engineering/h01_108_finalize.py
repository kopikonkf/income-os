...
def main():
 ...
 job={'schema':'die.h01.browser-job-result.v1','job_id':ns.job_id,'job_kind':'H01_108_PRODUCTION','provider_id':ns.provider,'profile_id':ns.profile_id,'udd_id':ns.udd_id,'terminal_state':'SUCCEEDED','completed_at':done,'classification':'NATIVE_SVG' if ns.source_kind=='FILE' else 'TEXT_SVG','provider_original_sha256':sha(original),'h01_103_status':'PASS','postproduction_status':post['result'],'semantic_asset_id':bp['semantic_asset_id'],'authority':{'provider_generation_dispatched':True,'submission_authorized':False,'publication_authorized':False}};dump(w/'browser-job-result.json',job)
