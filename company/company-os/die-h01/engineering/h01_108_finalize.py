...
def main():
 ...
 src=Path(ns.source);transport_normalization=None
 if ns.source_kind=='TEXT':
  raw=src.read_text().rstrip('\n');acq,_=acquire_svg_text(job_id=ns.job_id,request_id=req['request_id'],provider_id=ns.provider,profile_id=ns.profile_id,provider_status='COMPLETED',provider_response_text=raw,output_dir=f,completion_signal='SVG_COMPLETE_STABLE_NO_STOP',completed_at=done,finish_reason='PROVIDER_UI_TERMINAL');transport_normalization=acq.get('normalization');original=(f/'provider-original.svg').read_bytes()
  ...
  acq_path=f/'provider-output-acquisition.receipt.json'
  if acq_path.exists():
   acq=json.loads(acq_path.read_text());transport_normalization=acq.get('normalization');surface=acq.get('provider_surface') or {};surface_path=Path(surface.get('path','')) if surface.get('path') else None;raw=surface_path.read_text() if surface_path and surface_path.exists() else original.decode()
  else: raw=original.decode()
 ...
 h104=build_success_receipt(request=req,ingress='WEB_AI_ADAPTER',dispatch_commit_id=f'{ns.job_id}-CDP',provider_status='COMPLETED',provider_response_text=raw,h01_103_canonical_svg_sha256=norm['canonical_svg_sha256'],finish_reason='PROVIDER_UI_TERMINAL',candidate_svg_text=original.decode(),transport_normalization=transport_normalization);dump(f/'h01-104-native-svg-receipt.json',h104)
