...
def assert_unique_semantic_hash(runs_root:Path,digest:str,semantic_asset_id:str,current_workspace:Path)->None:
 for rec_path in runs_root.glob('*/artifact-created.receipt.json'):
  try:
   if rec_path.parent.resolve()==current_workspace.resolve(): continue
   rec=json.loads(rec_path.read_text())
  except Exception:
   continue
  if rec.get('provider_original_sha256')!=digest: continue
  prior_semantic=str(rec.get('semantic_asset_id') or '')
  if prior_semantic and prior_semantic!=semantic_asset_id:
   raise RuntimeError(f"E_CROSS_SEMANTIC_HASH_REUSE:{digest}:{prior_semantic}:{semantic_asset_id}:{rec_path.parent.name}")
...
def main():
 ...
 if ns.source_kind=='TEXT':
  raw=src.read_text().rstrip('\n');acq,_=acquire_svg_text(job_id=ns.job_id,request_id=req['request_id'],provider_id=ns.provider,profile_id=ns.profile_id,provider_status='COMPLETED',provider_response_text=raw,output_dir=f,completion_signal='SVG_COMPLETE_STABLE_NO_STOP',completed_at=done,finish_reason='PROVIDER_UI_TERMINAL');original=(f/'provider-original.svg').read_bytes();assert_unique_semantic_hash(w.parent,sha(original),bp['semantic_asset_id'],w);acq_method='TEXT_SVG'
 else:
  original=src.read_bytes();assert_unique_semantic_hash(w.parent,sha(original),bp['semantic_asset_id'],w);target=f/'provider-original.svg';
