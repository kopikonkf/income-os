...
| Legacy Azure Windows Hermes gateway tasks | **RETIRE / DO_NOT_MIGRATE** | H01 Linux Hermes | T-7 | H01 Hermes replacement is canonical DONE; do not migrate Azure Hermes profile/state/credentials. A separately installed Office Hermes utility is not Azure migration cargo and receives no control-plane authority from this row. | H01 Linux Hermes remains the proven runtime/compatibility edge. | `DIE-202/DIE-204` standing + `OPS-002` role disposition. |
| Legacy Azure Windows OpenCode Web Serve / OpenCodeBridge | **RETIRE / DO_NOT_MIGRATE** | Office shared OpenCode + H01 bounded executor | T-7 | Do not carry Azure user state merely for parity. MC-011D later accepted the rebuilt Office shared OpenCode `127.0.0.1:3000` surface and H01 OpenCode auth boundary; these are replacement surfaces, not surviving Azure copies. | Azure `:3000` is already `STOPPED_NO_RESPAWN`; replacement surfaces remain independently governed. | `MC-011D` + `OPS-002`. |
