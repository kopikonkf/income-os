...
class AttributionGrowthTests(unittest.TestCase):
    ...
    def test_growth_builds_seven_standard_work_cards_and_evidence_bound_atoms(self):
        d=self.data(); cards=growth.build_work_cards(commerce_package=d['commerce'],knowledge_package=d['kp'],campaign_key='organic-education-v1',listing_channel_id='gumroad'); self.assertEqual(len(cards),7); self.assertTrue(all(c['role']=='GROWTH_PRODUCER' for c in cards)); self.assertTrue(all(c['capability_requirements']=={'web_ai':True,'mcp':False,'shell':False,'local_filesystem':False} for c in cards))
        reg={c['channel_id']:c for c in growth.load_registry()['channels']}; outputs=self.fixture_outputs(); atoms=[]
        destinations=['google_play_books','gumroad','leanpub','fourthwall','ko_fi','lemon_squeezy','payhip']
        ...
        self.assertEqual({a['channel_id'] for a in atoms},{'x','threads','instagram','tiktok','youtube','pinterest','reddit'}); self.assertEqual(len({a['campaign_id'] for a in atoms}),1); self.assertEqual(len({a['attribution_identity']['listing_channel_id'] for a in atoms}),7); self.assertTrue(all(a['evidence_refs'] for a in atoms)); self.assertTrue(all(a['paid_ads'] is False and a['publication_authorized'] is False for a in atoms)); self.assertEqual(atoms[-1]['content']['cta'],'NONE')
        bp=growth.build_blueprint(commerce_package=d['commerce'],atoms=atoms,objective='organic education and demand capture'); self.assertEqual(len(bp['channels']),7); self.assertFalse(bp['paid_ads_enabled']); self.assertFalse(bp['publication_authorized'])
