...
function procProfile(profile){
  ...
  for(const e of entries){
    ...
    try{const s=fs.readFileSync(`/proc/${e}/cmdline`).toString().replace(/\0/g,' ');if(s.includes(needle)&&(s.includes('chrome')||s.includes('chromium')||s.includes('brave')))return Number(e)}catch{}
  }
  return null;
}
async function waitExit(child,ms){
  if(!child||child.exitCode!==null)return true;
  return await new Promise((resolve)=>{let done=false,timer;const finish=(v)=>{if(done)return;done=true;clearTimeout(timer);child.off('exit',onExit);resolve(v)};const onExit=()=>finish(true);child.once('exit',onExit);timer=setTimeout(()=>finish(false),ms)});
}
...
function publicHandle(h){return{principal_id:h.principalId,job_id:h.jobId,state:h.state,profile:h.cfg.profile,headful:true,founder_visible:true,display:FOUNDER_DISPLAY,workspace_index:WORKSPACE_BY_TARGET[h.principalId],workspace_number:WORKSPACE_BY_TARGET[h.principalId]+1,browser_pid:h.status?.browserPid||null,debug_host:h.status?.debugHost||null,debug_port:h.status?.debugPort||null,ready_state:h.status?.state||null,started_at:h.startedAt,credential_values_read:false,cookies_or_tokens_read:false}}
async function stopChildren(h){
  ...
  return{forced_process_cleanup:forced,profile_process_gone:!procProfile(h.cfg.profile),owner_exit_code:h.owner?.exitCode??null};
}
async function startRuntime(principalId,jobId){
  ...
  const workspaceIndex=WORKSPACE_BY_TARGET[principalId];if(!Number.isInteger(workspaceIndex))throw Object.assign(new Error('E_PRINCIPAL_WORKSPACE'),{httpStatus:500});
  ...
  if(prior){if(prior.jobId===jobId&&prior.state==='WORK')return{...publicHandle(prior),idempotent_replay:true};throw Object.assign(new Error(`E_PRINCIPAL_BROWSER_BUSY:${prior.jobId}`),{httpStatus:409})}
  const foreignPid=procProfile(cfg.profile);if(foreignPid)throw Object.assign(new Error(`E_PRINCIPAL_PROFILE_BUSY:${foreignPid}`),{httpStatus:409});
  ...
  const base={schema:'die.cognition.principal-browser-broker-lifecycle.v1',principal_id:principalId,job_id:jobId,profile:cfg.profile,headful:true,founder_visible:true,display:FOUNDER_DISPLAY,workspace_index:workspaceIndex,workspace_number:workspaceIndex+1,broker_socket:SOCKET,credential_values_read:false,cookies_or_tokens_read:false,started_at:startedAt,status:'SPAWNING'};
  atomicJson(receipt,base);
  let owner=null;const ownerErr=tailBuffer();
  try{
    owner=spawn('/usr/local/bin/node',[cfg.script,'launch'],{env:{...process.env,DISPLAY:FOUNDER_DISPLAY,HOME:'/home/kopiko'},stdio:['ignore','ignore','pipe']});
    owner.stderr?.on('data',ownerErr.push);
    const status=await waitStatus(cfg,started,owner);
    const placement=await placePidOnFounderWorkspace(principalId,status.browserPid);
    const h={principalId,jobId,cfg,owner,status,state:'WORK',startedAt,receipt,placement};active.set(principalId,h);
    atomicJson(receipt,{...base,status:'WORK',browser_pid:status.browserPid,debug_host:status.debugHost,debug_port:status.debugPort,ready_state:status.state,placement,ready_at:new Date().toISOString()});
    if(status.state!=='READY'){
      ...
      atomicJson(receipt,{...base,status:'COLD',ready_state:status.state,auth_repair_required:true,repair_hold_path:holdPath,repair_hold_state:repair.state,placement,owner_stderr_tail:ownerErr.get(),...stopped,completed_at:new Date().toISOString()});
      const authError=Object.assign(new Error(`E_AUTH_REPAIR_REQUIRED:${principalId}:${status.state}`),{httpStatus:423,details:{repair_hold:repair},lifecycleHandled:true});throw authError;
    }
    return publicHandle(h);
  }catch(error){
    if(!active.has(principalId)&&!error?.lifecycleHandled){
      const h={principalId,jobId,cfg,owner,status:null,state:'FAILED',startedAt,receipt};
...
async function stopRuntime(principalId,jobId,terminalPath,reason='COGNITION_TERMINAL'){
  const cfg=CFG[principalId];if(!cfg)throw Object.assign(new Error('E_PRINCIPAL_BROWSER_PRINCIPAL'),{httpStatus:400});
  const h=active.get(principalId);if(!h)throw Object.assign(new Error('E_PRINCIPAL_BROWSER_NOT_ACTIVE'),{httpStatus:409});
  if(h.jobId!==jobId)throw Object.assign(new Error(`E_PRINCIPAL_BROWSER_JOB_MISMATCH:${h.jobId}`),{httpStatus:409});
  const terminal=terminalEvidence(cfg,terminalPath);h.state='STOPPING';const stopped=await stopChildren(h);active.delete(principalId);
  const out={schema:'die.cognition.principal-browser-broker-lifecycle.v1',principal_id:principalId,job_id:jobId,profile:cfg.profile,headful:true,founder_visible:true,display:FOUNDER_DISPLAY,workspace_index:WORKSPACE_BY_TARGET[principalId],workspace_number:WORKSPACE_BY_TARGET[principalId]+1,broker_socket:SOCKET,status:stopped.profile_process_gone?'COLD':'CLOSE_FAILED',close_reason:reason,browser_pid:h.status?.browserPid||null,debug_host:h.status?.debugHost||null,debug_port:h.status?.debugPort||null,placement:h.placement||null,terminal_evidence_path:terminal.path,terminal_evidence_sha256:terminal.sha256,terminal_schema:terminal.schema,terminal_status:terminal.status,...stopped,credential_values_read:false,cookies_or_tokens_read:false,started_at:h.startedAt,completed_at:new Date().toISOString()};
  atomicJson(h.receipt,out);if(out.status!=='COLD')throw Object.assign(new Error('E_PRINCIPAL_BROWSER_CLOSE_INCOMPLETE'),{httpStatus:500});return out;
}
async function readBody(req){let raw='';for await(const chunk of req){raw+=chunk;if(raw.length>16384)throw Object.assign(new Error('E_REQUEST_TOO_LARGE'),{httpStatus:413})}if(!raw)return{};try{return JSON.parse(raw)}catch{throw Object.assign(new Error('E_REQUEST_JSON'),{httpStatus:400})}}
...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/v1/start'){const b=await readBody(req),out=await startRuntime(b.principal_id,b.job_id);return send(res,200,{schema:'die.cognition.principal-browser-broker-start.v1',status:'READY',runtime:out})}
    if(req.method==='POST'&&url.pathname==='/v1/stop'){const b=await readBody(req),out=await stopRuntime(b.principal_id,b.job_id,b.terminal_evidence_path,b.reason||'COGNITION_TERMINAL');return send(res,200,{schema:'die.cognition.principal-browser-broker-stop.v1',status:'COLD',runtime:out})}
...
server.listen(SOCKET,()=>{fs.chmodSync(SOCKET,0o660);console.log(JSON.stringify({schema:'die.cognition.principal-browser-broker.v1',state:'READY',socket:SOCKET,display:FOUNDER_DISPLAY,principals:Object.keys(CFG),credential_values_read:false,cookies_or_tokens_read:false}))});
async function shutdown(){server.close();for(const h of [...active.values()]){await stopChildren(h).catch(()=>{});active.delete(h.principalId)}fs.rmSync(SOCKET,{force:true});process.exit(0)}
