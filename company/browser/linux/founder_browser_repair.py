...
CFG={
 'cluster-a':{'display':101,'broker':39121,'view_service':'die-founder-vnc-cluster-a.service','interactive_service':'die-founder-repair-vnc-cluster-a.service','interactive_port':59201},
 'cluster-b':{'display':102,'broker':39122,'view_service':'die-founder-vnc-cluster-b.service','interactive_service':'die-founder-repair-vnc-cluster-b.service','interactive_port':59202},
}
...
def open_repair(cluster:str,timeout:int)->int:
 ...
 try:
  subprocess.run(['sudo','systemctl','start',cfg['interactive_service']],check=True)
  atomic(REPAIR_ROOT/f'{cluster}.runtime.json',{'service':cfg['interactive_service'],'cluster_id':cluster,'interactive_port':cfg['interactive_port'],'started_at':now()})
  deadline=time.time()+timeout
  while time.time()<deadline:
   active=subprocess.run(['systemctl','is-active','--quiet',cfg['interactive_service']]).returncode==0
   if not active:raise RuntimeError('E_INTERACTIVE_VNC_STOPPED')
   time.sleep(1)
 finally:
  subprocess.run(['sudo','systemctl','stop',cfg['interactive_service']],check=False)
