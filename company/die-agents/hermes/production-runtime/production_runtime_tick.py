...
def generate(w:Path)->dict:
 ...
 rec=json.loads(cp.stdout.strip().splitlines()[-1]);src=Path(rec['artifact_path'])
 if rec.get('status')!='SUCCEEDED' or not src.is_file() or sha(src)!=rec.get('sha256'):raise RuntimeError('E_MUXIA_RECEIPT')
 pdir=w/'provider';pdir.mkdir(exist_ok=True);ext=src.suffix.lower() or '.png';dst=pdir/('source-original'+ext);shutil.copy2(src,dst);shutil.copy2(Path('/var/lib/muxia/state/receipts')/f'{w.name}-muxia.json',pdir/'muxia-receipt.json')
