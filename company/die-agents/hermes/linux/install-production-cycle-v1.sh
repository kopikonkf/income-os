...
PROMPT='Run exactly one DIE production-cycle tick using company/operations/PRODUCTION_CHAIN_OPERATING_PLAYBOOK_V1.md as the operating playbook. Continue an unfinished production card before starting new work. Start at most one new approved Object Atlas seed noun this tick. Generate per seed noun and manage/reuse Blueprint at family level. Reuse a still-valid fixed Blueprint; call Division01 only when semantic authoring/revision is actually required; call Executive only for a new/material family promotion or escalation, never as a per-image gate. Hermes owns Kanban/state and must dispatch bounded Worker jobs rather than doing Worker work itself. Use the currently available production provider (MUXIA or another explicitly configured provider) only within zero-spend existing authority. A provider completion is not complete until a real artifact exists on filesystem. Continue immediately through recovery/upscale when needed, metadata, rights/IP preflight, deterministic QA and QC; do not wait for the next 3-hour tick once a cycle has started. Stop at WAITING_FOUNDER_QC / READY_FOR_MANUAL_PUBLISH; do not upload, submit, publish, spend, or perform account/credential actions without Founder authority. During this early operational phase use `hermes send --to telegram` to report PRODUCTION_STARTED, ARTIFACT_CREATED, QA_QC_UPDATE, WAITING_FOUNDER_QC, READY_FOR_MANUAL_PUBLISH, or an explicit FAILED/BLOCKED milestone with task id, stage and next action. Silent failure is forbidden. The atomic task graph remains the long-horizon engineering journey; unrelated unfinished nodes are not production blockers. Return a concise final cycle summary.'
