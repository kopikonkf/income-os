...
runuser -u "$SERVICE_USER" -- env HERMES_HOME="$HERMES_HOME" DIE_HOME="$DIE_HOME" DIE_STATE_ROOT="$DIE_STATE_ROOT" TERMINAL_CWD="$WORKDIR" "$HERMES_BIN" cron create "$SCHEDULE" "$PROMPT" --name "$JOB_NAME" --deliver telegram --workdir "$WORKDIR" --no-continuity
