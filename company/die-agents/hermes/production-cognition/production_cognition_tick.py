...
MAX_SEMANTIC_ATTEMPTS=3; MAX_REVISIONS=2; MAX_CONTEXT_RETRIES=3
...
def _context_instruction(principal:str,req_id:str,repo_sha:str)->str:
 return f'''Attempt Runtime MCP `context_snapshot` first. If it is available, require the correct principal/scope, fresh context, VERIFIED canon, and repository revision `{repo_sha}`. If Runtime MCP is unavailable solely because the connector/account/MCP connection is unavailable, you MAY proceed in `SEALED_CONTEXT_FALLBACK`: use only the exact repository SHA, hashes, artifacts and evidence embedded in this Hermes request; do not assert any newer canon state, external evidence, credentials, or authority beyond the envelope. A tool-observed mismatch, ambiguous principal binding, hash mismatch, or evidence inconsistency must still fail closed. Only if neither verified Runtime MCP nor the sealed envelope can safely support the task, return exactly one JSON object: {{"schema":"die.cognition.blocked.v1","request_id":"{req_id}","principal_id":"{principal}","reason_code":"E_CONTEXT_CONVERGENCE","reason":"bounded explanation"}}.'''
...
def record_transport_retry(state:dict[str,Any],*,stage:str,request_id:str,lane:str,reason:str)->dict[str,Any]:
 ...
 if lane=='AUTHOR':
  ...
  state['history'].append({'at':now(),'event':event,'request_id':request_id,'lane':'AUTHOR','reason':reason,'attempt':n,'retryable':retryable})
 ...
 if lane=='REVIEW':
  ...
  state['history'].append({'at':now(),'event':event,'request_id':request_id,'lane':'REVIEW','reason':reason,'attempt':n,'retryable':retryable})
  return {'retryable':retryable,'attempt':n,'stage':state['stage']}
 if lane=='SUBJECT':
  ...
  state['history'].append({'at':now(),'event':event,'request_id':request_id,'lane':'SUBJECT','reason':reason,'attempt':n,'retryable':retryable})
  return {'retryable':retryable,'attempt':n,'stage':state['stage']}
...
def tick(args)->dict[str,Any]:
 ...
 if stage=='NEED_REVIEW':
  ...
  if parsed.get('schema')=='die.cognition.blocked.v1':
   ...
   if be: raise RuntimeError('E_BLOCKED_RESPONSE_BINDING:'+','.join(be))
   reason=parsed.get('reason_code')
   if reason=='E_CONTEXT_CONVERGENCE' and review_attempt<MAX_CONTEXT_RETRIES:
    review_attempt+=1; state['review_attempt']=review_attempt; state['stage']='NEED_REVIEW'; state['history'].append({'at':now(),'event':'PRINCIPAL_BLOCKED','request_id':rid,'reason':reason,'retryable':True,'next_review_attempt':review_attempt}); atomic_json(statep,state); return {'schema':'die.production.cognition-tick.v1','status':'RETRY','task_id':task,'reason':reason,'review_attempt':review_attempt}
   state['stage']='WAITING_FOUNDER'; state['history'].append({'at':now(),'event':'PRINCIPAL_BLOCKED','request_id':rid,'reason':reason,'retryable':False}); atomic_json(statep,state); return {'schema':'die.production.cognition-tick.v1','status':'BLOCKED','task_id':task,'reason':reason}
