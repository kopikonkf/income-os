...
def validate(request:dict[str,Any], *, projection:dict[str,Any]) -> dict[str,Any]:
    ...
    if request['action_type'].startswith('OP-REQUEST-DIVISION01') and request.get('target_principal_id')!='division-head-division01': errors.append('E_TARGET_PRINCIPAL')
    if request['action_type'].startswith('OP-REQUEST-EXECUTIVE') and request.get('target_principal_id')!='chatgpt-plus-executive': errors.append('E_TARGET_PRINCIPAL')
