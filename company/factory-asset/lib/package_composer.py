...
def compose_dry_run_package(*,package_dir:str|Path,semantic_asset_id:str,master_sha256:str,deliverables:list[dict[str,Any]],metadata_ref:str,rights_ref:str,compatibility_receipt_ref:str)->dict[str,Any]:
    ...
    return {'schema':'die.factory-asset.marketplace-dry-run-package-receipt.v1','result':'PASS','manifest_sha256':manifest_sha,'manifest_path':str(root/'manifest.json'),'file_count':len({x['package_path'] for x in items}),'unique_file_count':len({x['package_path'] for x in items}),'manifest_entry_count':len(items),'semantic_asset_count':1,'derivative_count':len(items),'publication_action':'NONE','upload_action':'NONE'}