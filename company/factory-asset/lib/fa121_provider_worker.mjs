...
function assertProviderConfig(providerId, providerConfig) {
  if (!['qwen', 'chatgpt'].includes(providerId)) throw new Error(`E_FA121_PROVIDER_UNSUPPORTED:${providerId}`);
  if (!providerConfig || providerConfig.actual_live_transport !== 'BROWSER_CDP') throw new Error('E_FA121_TRANSPORT_CONFIG');
  if (providerId === 'qwen' && providerConfig.session_api_live_executor_claimed !== false) throw new Error('E_FA121_QWEN_TRANSPORT_TRUTH');
}
...
export async function generateFa121ProviderImage({ controlBaseUrl, playwrightEntry, providerId, providerConfig, readinessProfile, jobId, prompt, artifactDir, journalPath = null, ttlMs = 600000, timeoutMs = 300000 }) {
  assertProviderConfig(providerId, providerConfig);
  ...
  try {
    ...
    if (lease?.lease_id) {
      const release = await releaseClusterTab(controlBaseUrl, lease.lease_id, 'FA121_ATTEMPT_COMPLETE').catch((error) => ({ released: false, error: safeError(error) }));
