...
for f in company/browser/linux/job_scoped_cluster_runtime.mjs company/browser/linux/principal_job_browser_runtime.mjs company/browser/linux/principal_browser_broker.mjs company/browser/linux/die-principal-browser-broker.service company/browser/linux/founder_no_cdp_repair.sh company/factory-asset/bin/production_multi_cluster_dispatch.mjs; do
...
install -d -o kopiko -g die-runtime -m 2770 /var/lib/die/state/principal-auth-repair
...
systemctl enable --now die-principal-browser-broker.service
...
[[ -S /run/die/principal-browser-broker.sock ]] || { echo E_FA338_PRINCIPAL_BROKER_SOCKET >&2; exit 3; }
printf '%s\n' 'FA338_CUTOVER=PASS' 'PRINCIPAL_BROWSER_BROKER=READY' 'OWNER_MODEL=JOB_SCOPED_HEADFUL_BROWSER_CDP' 'LEGACY_OWNER_UNITS=DISABLED_ROLLBACK_RETAINED' 'IDLE_BROWSER_OWNERS=0'
