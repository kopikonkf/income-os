  ...
  function renderQC() {
    ...
    $("#view-qc").innerHTML = `
      <article class="card" style="margin-bottom:16px"><div class="result-head"><div><h2>Founder QC Gallery</h2><p class="muted">One review image per accepted canary/job; read-only bulk inspection</p></div>${d ? badge('READ ONLY') : badge('UNKNOWN')}</div>
        ...
        ${d ? `<div class="qc-toolbar"><input id="qc-search" placeholder="Search seed, job, provider…" value="${esc(state.qcSearch)}"><select id="qc-source"><option value="ALL" ${state.qcSource==='ALL'?'selected':''}>All (${d.asset_count})</option><option value="FA124_CANARY" ${state.qcSource==='FA124_CANARY'?'selected':''}>FA-124 (${d.source_counts.FA124_CANARY||0})</option><option value="H01_108_VECTOR_SOAK" ${state.qcSource==='H01_108_VECTOR_SOAK'?'selected':''}>H01-108 Vector Soak (${d.source_counts.H01_108_VECTOR_SOAK||0})</option><option value="PRODUCTION_WORKSPACE" ${state.qcSource==='PRODUCTION_WORKSPACE'?'selected':''}>Production Workspaces (${d.source_counts.PRODUCTION_WORKSPACE||0})</option></select><span class="badge violet">${rows.length} visible</span></div>` : ''}
      </article>
      <div class="qc-grid">${rows.map(x=>`<button class="qc-card" data-qc-id="${esc(x.asset_id)}"><img loading="lazy" src="${esc(x.thumb_url)}" alt="${esc(x.seed_name)}"><span class="qc-card-body"><strong>${esc(x.seed_name)}</strong><small>${esc(x.provider_route||'—')} · ${esc(x.review_kind)}</small><small>${esc(x.job_id)} · ${esc(x.qc_state)}${x.rights_state?` · rights:${esc(x.rights_state)}`:''}</small></span></button>`).join('')}</div>
      ${selected ? `<div class="qc-lightbox" id="qc-lightbox"><button class="qc-close" id="qc-close">×</button><div class="qc-stage"><img src="${esc(selected.full_url)}" alt="${esc(selected.seed_name)}"></div><aside class="qc-meta"><h2>${esc(selected.seed_name)}</h2><dl class="kv"><dt>Job</dt><dd>${esc(selected.job_id)}</dd><dt>Source</dt><dd>${esc(selected.source_group)}</dd><dt>Provider</dt><dd>${esc(selected.provider_route||'—')}</dd><dt>Model</dt><dd>${esc(selected.model_name||'UNKNOWN')}</dd><dt>Model version</dt><dd>${esc(selected.model_version||'UNKNOWN')}</dd><dt>Model evidence</dt><dd>${esc(selected.model_evidence||'UNKNOWN')}</dd><dt>Review kind</dt><dd>${esc(selected.review_kind)}</dd><dt>QC state</dt><dd>${badge(selected.qc_state)}</dd><dt>Rights</dt><dd>${badge(selected.rights_state||'NOT_RECORDED')}</dd><dt>Postproduction</dt><dd>${badge(selected.postproduction_state||'NOT_RECORDED')}</dd><dt>Format</dt><dd>${esc(selected.format||'—')}</dd><dt>Dimensions</dt><dd>${selected.width_px&&selected.height_px?`${selected.width_px}×${selected.height_px}`:'—'}</dd><dt>Bytes</dt><dd>${Number(selected.bytes||0).toLocaleString()}</dd></dl><p class="notice">Read-only review. No Founder QC decision is written from this gallery yet.</p></aside></div>` : ''}`;
