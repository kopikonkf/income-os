...
def test_threshold_contract_has_large_actual_to_control_margin():
    ...
    control_wm=vr.classify_watermark(full_scores=[0.22210145,0.49236092,0.28502241,0.00051525],cfg=cfg['watermark'])
    ...
    assert control_wm['disposition']=='REVIEW_CANDIDATE' and control_wm['positive_score']>0.25
    assert control_wm['scoring']=='MAX_RISK_MINUS_CLEAN'
    ...
    assert branded_ip['disposition']=='STRONG_RISK' and branded_ip['risk_score']>0.80
...
def test_source_ip_applicability_prevents_irrelevant_property_class_bias():
    cfg=vr.load_config()['source_ip']
    full=[0.32004085,0.114009,0.02867694,0.04337548,0.49389774]
    fg=[0.23737279,0.09823223,0.03061331,0.03791143,0.59587026]
    isolated=vr.classify_source_ip(full_scores=full,foreground_scores=fg,cfg=cfg,asset_type='ISOLATED_OBJECT')
    photo=vr.classify_source_ip(full_scores=full,foreground_scores=fg,cfg=cfg,asset_type='PHOTO')
    assert isolated['disposition']=='CLEAR' and isolated['risk_score']==0.0
    assert isolated['applicable_risk_indices']==[1,2]
    assert photo['disposition']=='STRONG_RISK' and photo['risk_score']>0.30
    assert photo['applicable_risk_indices']==[1,2,3,4]
    assert isolated['scoring']=='MAX_APPLICABLE_RISK_MINUS_CLEAN'
