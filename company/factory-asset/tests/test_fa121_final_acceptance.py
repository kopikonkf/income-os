...
def test_fa121_receipt_and_graph_close_soak_and_unlock_dependencies():
 r=json.loads(R.read_text());g=json.loads(G.read_text());by={x["id"]:x for x in g["tasks"]};assert r["status"]=="DONE" and r["result"]=="PASS" and r["full_fa121_acceptance"] is True;assert by["FA-121"]["status"]=="DONE";assert by["FA-122"]["status"]=="READY";assert by["FA-307"]["status"] in {"READY","IN_PROGRESS","DONE"}
