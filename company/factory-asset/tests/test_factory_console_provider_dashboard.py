...
def test_provider_dashboard_endpoint_state_is_live_when_brokers_reachable_or_fixture_fallback():
 m=load_server();d=m.provider_dashboard_state();assert d['schema']=='die.factory-asset.provider-dashboard.v1';assert d['evidence_mode'] in {'LIVE_BROKER_SANITIZED','SYNTHETIC_OBSERVED_FIXTURE'};assert d['guessed_quota_present'] is False;assert d['provider_dispatch_performed'] is False
 if d['evidence_mode']=='LIVE_BROKER_SANITIZED':
  assert len(d['providers'])==10;assert {x['cluster_id'] for x in d['providers']}=={'cluster-a','cluster-b'};assert all(x['transport']=='BROWSER_CDP' for x in d['providers'])
 else:
  assert d['selected_profile_id']=='qwen_a';rows={x['provider_id']:x for x in d['providers']};assert rows['grok']['eligibility']=='DEFERRED_OPTIONAL'
...
def test_provider_dashboard_http_roundtrip():
 ...
 try:
  ...
  assert d['provider_dispatch_performed'] is False;assert len(d['providers']) in {6,10};assert d['evidence_mode'] in {'LIVE_BROKER_SANITIZED','SYNTHETIC_OBSERVED_FIXTURE'}
...
def test_optional_grok_does_not_block_selected_route():
 m=load_server();d=m.provider_dashboard_state()
 if d['evidence_mode']=='SYNTHETIC_OBSERVED_FIXTURE':
  rows={x['provider_id']:x for x in d['providers']};assert rows['grok']['eligibility']=='DEFERRED_OPTIONAL';assert d['selected_profile_id'] is not None and d['selected_profile_id']!='grok_a'
 else:
  assert all(x['provider_id']!='grok' for x in d['providers'])