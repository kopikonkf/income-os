...
def test_gallery_uses_e4_accepted_truth_and_one_workspace_representative(tmp_path):
 ...
 m=load('qc_unit',LIB);h01=tmp_path/'h01';d=m.build_gallery(repo,workspaces_root=work,fa124_root=fa,h01_108_root=h01);assert d['asset_count']==2 and d['source_counts']=={'FA124_CANARY':1,'PRODUCTION_WORKSPACE':1};assert all('_path' not in x for x in d['items']);a=d['items'][0];body,ctype=m.image_payload(repo,a['asset_id'],'full',workspaces_root=work,fa124_root=fa,h01_108_root=h01);assert body==b'PNGDATA' and ctype=='image/png'
...
def test_h01_108_vector_soak_preview_is_dynamic_read_only_gallery_source(tmp_path):
 repo=tmp_path/'repo';c=repo/'company/factory-asset/fixtures/scale';c.mkdir(parents=True);(c/'FA-124-cartoon-watercolor-cohort.json').write_text(json.dumps({'jobs':[]}))
 fa=tmp_path/'fa';fa.mkdir();(fa/'FA124-E4-final.json').write_text(json.dumps({'technical_qa':[]}))
 work=tmp_path/'work';work.mkdir();h01=tmp_path/'h01';run=h01/'042-mug-qwen-a1';post=run/'postproduction';post.mkdir(parents=True);(post/'preview.webp').write_bytes(b'WEBPDATA')
 (run/'batch-item.json').write_text(json.dumps({'canonical_name':'mug','source_candidate_id':'CAND-42'}));(run/'asset-receipt.json').write_text(json.dumps({'status':'ACCEPTED_SEMANTIC_MASTER','provider_id':'qwen','noun':'mug'}));(run/'rights-signal.json').write_text(json.dumps({'result':'REVIEW_REQUIRED'}));(run/'postproduction-state.json').write_text(json.dumps({'status':'PARKED_FOUNDER_QC','rights':'REVIEW_REQUIRED','updated_at':'2026-09-13T11:00:00Z'}))
 m=load('qc_h01_vector',LIB);d=m.build_gallery(repo,workspaces_root=work,fa124_root=fa,h01_108_root=h01);assert d['asset_count']==1 and d['source_counts']=={'H01_108_VECTOR_SOAK':1}
 a=d['items'][0];assert a['seed_name']=='mug' and a['provider_route']=='qwen' and a['qc_state']=='ACCEPTED_SEMANTIC_MASTER' and a['rights_state']=='REVIEW_REQUIRED' and a['postproduction_state']=='PARKED_FOUNDER_QC' and a['format']=='WEBP'
 body,ctype=m.image_payload(repo,a['asset_id'],'full',workspaces_root=work,fa124_root=fa,h01_108_root=h01);assert body==b'WEBPDATA' and ctype=='image/webp'
