...
def adapt(raw: dict[str, Any]) -> dict[str, Any]:
    required={"source_id","source_name","query","market_locale","language","buyer_term_presence","observed_at","freshness_window_seconds"}
    missing=sorted(required-raw.keys())
    if missing: raise ValueError("missing:"+",".join(missing))
    value=raw["buyer_term_presence"]
    if not isinstance(value,bool): raise ValueError("buyer_term_presence must be boolean")
    ...
    recorded=observed+dt.timedelta(seconds=20)
    ...
    return {
      "schema_version":"die.division001.opportunity-signals.v1","signal_id":make_signal_id("BUYER",fingerprint),
      ...
      "signal_class":"COMMERCIAL_INTENT","signal_type":"BUYER_TERM_PRESENCE",
      "value":{"kind":"BOOLEAN","numeric_value":None,"boolean_value":value,"text_value":None,"unit":"presence"},
      ...
      "evidence_label":"SYNTHETIC","confidence":"HIGH","collector_id":"fixture-adapter:buyer-intent:v1","acquisition_method":"SYNTHETIC_FIXTURE",
      "policy":{"profile_id":"fixture-buyer-intent","profile_version":"v1","classification":"SYNTHETIC_ONLY"},
      "source_ref":"fixture://buyer-intent/"+fingerprint,"evidence_sha256":fingerprint,"cost_usd":0,
      "dedupe_key":make_dedupe_key(raw["source_id"],subject_id,"BUYER_TERM_PRESENCE",observed_z),
    }
