...
UNIT_SRC="$STAGING_DIE_HOME/company/division/division001/linux/die-division01-runtime-mcp-staging.service"
UNIT_DST="/etc/systemd/system/die-division01-runtime-mcp-staging.service"
ENV_DIR="$DIE_CONFIG_ROOT/staging/division01"
...
DIE_MCP_BASE_URL=https://division01-h01-mcp.aethers.web.id
DIE_MCP_OAUTH_CLIENT_ID=chatgpt-die-lnx-division-001
...
grep -qx 'DIE_MCP_BASE_URL=https://division01-h01-mcp.aethers.web.id' "$ENV_FILE" || { echo "E_STAGING_BASE_URL_MISMATCH" >&2; exit 2; }
grep -qx 'DIE_MCP_CONTROL_POLICY=staging-read-only' "$ENV_FILE" || { echo "E_STAGING_CONTROL_POLICY" >&2; exit 2; }

install -m 0644 "$UNIT_SRC" "$UNIT_DST"
systemctl daemon-reload
systemctl disable "die-division01-runtime-mcp-staging.service" >/dev/null 2>&1 || true
systemctl stop "die-division01-runtime-mcp-staging.service" >/dev/null 2>&1 || true

echo "MCP_LNX_STAGING_INSTALL=PASS"
echo "SERVICE=die-division01-runtime-mcp-staging.service"
