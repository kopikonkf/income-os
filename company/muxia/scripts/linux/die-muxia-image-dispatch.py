...
NODE='/usr/local/bin/node'; XVFB='/usr/bin/xvfb-run'; MUXIA_USER='kopiko'
...
def export_verified(task:str,r:dict)->dict:
 ...
 if provider.stat().st_gid!=gid: raise RuntimeError('E_PROVIDER_GROUP')
 ...
 if tmp.stat().st_gid!=gid: tmp.unlink(missing_ok=True); raise RuntimeError('E_EXPORT_GROUP')
 ...
 shutil.copyfile(rp,rt)
 if rt.stat().st_gid!=gid: rt.unlink(missing_ok=True); raise RuntimeError('E_RECEIPT_GROUP')
 os.chmod(rt,0o640); os.replace(rt,rd)
...
def main()->int:
 ...
 if os.geteuid()!=pwd.getpwnam(MUXIA_USER).pw_uid:
  print('E_SERVICE_IDENTITY',file=sys.stderr);return 2
 ...
 cmd=[XVFB,'-a',NODE,str(RUNNER),'--job-id',f'{task}-muxia','--profile','chatgpt-linux-a','--prompt',prompt,'--timeout-ms','600000']
