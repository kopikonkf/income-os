...
`production_active_card_resolver.py` classifies durable production workspaces before the production LLM wakes and separately declares whether the resolved action has a canonical autonomous execution surface. `BLUEPRINT_REQUIRED` is delegated to `PRODUCTION_COGNITION_LINE_V1`; the production preflight returns `WAITING_COGNITION` + `wakeAgent=false` until that line emits a validated fixed Blueprint. Explicit state is read from `PROGRESS.md`; the accepted pre-state-field shopping-bag canary is recognized only through its real artifact plus `PENDING_FOUNDER` rights-review evidence and is classified as `PARKED_HUMAN_GATE`. Non-production workspaces without a production state are ignored. Unknown explicit production states fail closed. For an actionable card the resolver emits exact `task_id`, `state`, seed/family context, `required_actor`, `next_action_type`, and a bounded next-action instruction. This orientation neither authors semantics nor grants authority. If `execution_ready=false`, Hermes MUST report the exact execution blocker and MUST NOT search for, synthesize or improvise an alternate invocation route. Founder authorization on 2026-09-02 enables the governed internal cognition transport for Division01 and Executive prompt submit/output extraction only; this does not grant publication, submission, spend, credential, or account-configuration authority.
