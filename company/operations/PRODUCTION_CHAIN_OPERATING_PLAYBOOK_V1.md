...
`production_active_card_resolver.py` classifies durable production workspaces before the production LLM wakes and separately declares whether the resolved action has a canonical autonomous execution surface. Explicit state is read from `PROGRESS.md`; the accepted pre-state-field shopping-bag canary is recognized only through its real artifact plus `PENDING_FOUNDER` rights-review evidence and is classified as `PARKED_HUMAN_GATE`. Non-production workspaces without a production state are ignored. Unknown explicit production states fail closed. For an actionable card the resolver emits exact `task_id`, `state`, seed/family context, `required_actor`, `next_action_type`, and a bounded next-action instruction. This orientation neither authors semantics nor grants authority. If `execution_ready=false`, Hermes MUST report the exact execution blocker and MUST NOT search for, synthesize or improvise an alternate invocation route. For the current Linux Division01 Blueprint path, the canonical wake transport supports focus/stage/canary but not autonomous submit/output extraction; until a governed outbox-to-wake cognition handoff is wired, `BLUEPRINT_REQUIRED` resolves as `BLOCKED_ACTIVE_CARD` with `E_DIVISION01_AUTONOMOUS_COGNITION_TRANSPORT_UNWIRED`.
